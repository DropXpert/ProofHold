  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between pt-1">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-widest text-muted">Marketplace</p>
          <h1 className="text-[18px] font-bold tracking-tight text-ink leading-tight">Browse listings</h1>
        </div>
        {session && (
          <Link to="/listings/new" className="btn-primary flex items-center gap-1.5 px-3.5 py-2 text-[11px]">
      ...
      <form onSubmit={handleSearch} className="relative">
        ...
        <input
          className="input pl-8 pr-20 text-[12px]"
          placeholder="Search services..."
          value={search}
          onChange={(e) => setSearch(e.target.value)}
        />
        <button type="submit" className="absolute right-2 top-1/2 -translate-y-1/2 rounded-md bg-accent px-2.5 py-1 text-[10.5px] font-medium text-white">
          Search
        </button>
      </form>
      ...
      {loading ? (
        ...
        <div className="card px-5 py-10 flex flex-col items-center gap-4 text-center">
          ...
          <div className="space-y-1">
            <p className="text-[12px] font-semibold text-ink">No listings yet</p>
            <p className="text-[10.5px] text-muted">Be the first to list your service.</p>
          </div>
          {session && (
          ...
          {listings.map((l) => (
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <p className="text-[12px] font-semibold text-ink leading-snug line-clamp-2 flex-1">
                  {l.title}
                </p>
                <p className="shrink-0 text-[13px] font-bold tabular-nums text-ink">
                  {l.priceAmount}
                  <span className="text-[10px] font-medium text-muted ml-1">{l.priceCurrency}</span>
                </p>
              </div>
