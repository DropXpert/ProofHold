  ...
  if (!listing) {
    return (
      <div className="space-y-5">
        ...
        <div className="card px-5 py-10 text-center text-[12px] text-muted">
    ...
    <div className="space-y-5">
      ...
      <section className="card px-5 py-5 space-y-4">
        <div className="flex items-start justify-between gap-3">
          <h1 className="text-[17px] font-bold leading-snug text-ink flex-1">{l.title}</h1>
          <div className="shrink-0 text-right">
            <p className="text-[19px] font-bold tabular-nums text-ink leading-none">{l.priceAmount}</p>
            <p className="text-[11.5px] text-muted mt-0.5">{l.priceCurrency}</p>
          </div>
        </div>
        ...
        {l.description && (
          <p className="text-[12.5px] leading-relaxed text-ink whitespace-pre-wrap">{l.description}</p>
        )}
      ...
      <section className="card px-5 py-4 flex items-center gap-3">
        ...
        <Link to={`/profile/${encodeURIComponent(l.sellerAddr)}`} className="btn-secondary text-[11.5px] px-3 py-2 shrink-0">
          View profile
        </Link>
      </section>
      ...
      <section className="card px-5 py-4 space-y-2">
        <p className="text-[12px] font-semibold text-ink">What counts as delivery</p>
        <p className="text-[12px] text-muted leading-relaxed">{l.requiredDeliveryProof}</p>
      </section>

      <section className="card px-5 py-4 space-y-2">
        <p className="text-[12px] font-semibold text-ink">Refund terms</p>
        <p className="text-[12px] text-muted leading-relaxed">{l.refundTerms}</p>
      </section>
      ...
      {!isOwner && (
        <div className="space-y-3">
          ...
          {offerError && <p className="text-[11.5px] text-danger">{offerError}</p>}
          ...
          {(!myOffer || myOffer.status !== "accepted") && !showOfferForm && (
            <div className="space-y-2">
              ...
              {buyError && <p className="text-[11.5px] text-danger">{buyError}</p>}
              {!session && (
                <p className="text-center text-[11.5px] text-muted">Connect your wallet to buy or make an offer.</p>
              )}
              {session && (
                <p className={`text-center text-[11.5px] ${sellerDealLimitReached ? "text-danger" : "text-muted"}`}>
                  {sellerDealLimitReached
                    ? ACTIVE_DEAL_LIMIT_MESSAGE
        ...
        <>
          ...
          <section className="card px-5 py-4 space-y-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-[13px] font-semibold text-ink">Offers</h3>
              {incoming.length > 0 && (
            ...
            {incoming.length === 0 ? (
              <p className="text-[12px] text-muted">No offers yet. They'll appear here in real time.</p>
            ) : (
          ...
          {/* Manage listing */}
          <section className="card px-5 py-4 space-y-3">
            <p className="text-[12px] font-semibold text-ink">Manage listing</p>
            <div className="grid grid-cols-2 gap-2">
              ...
              {confirmingDelete ? (
                <button type="button" onClick={handleDelete} className="btn-danger w-full text-[12px]">
                ...
                <button
                  ...
                  className="btn-danger w-full text-[12px]"
                >
                  <Trash2 className="h-4 w-4" />
                  Delete
                </button>
            ...
            {confirmingDelete && (
              <button
                ...
                className="text-[11.5px] text-muted hover:text-ink transition"
              >
                Cancel
              </button>
            )}
            <div className={`rounded-lg px-3 py-2 text-[11.5px] text-center ${l.status === "active" ? "bg-accent-soft text-accent-ink" : "bg-warning/10 text-warning"}`}>
