...
export default function CreateListing() {
  ...
  useEffect(() => {
    if (session?.address) { setSellerAddr(session.address); return; }
    getWallet(priceCurrency).then((w) => w.getAddress()).then(setSellerAddr).catch(() => {});
  }, [session?.address, priceCurrency]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!sellerAddr) return setError("Connect a wallet first.");
    ...
    <div className="space-y-5">
      ...
      <form onSubmit={submit} className="space-y-5">
        <section className="card space-y-4 px-5 py-5">
          ...
          <p className="rounded-lg border border-dashed border-edge bg-bg px-3 py-2 text-[12.5px] leading-relaxed text-muted">
            ...
            <div className="space-y-2">
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {tags.map((t) => (
                    <span key={t} className="flex items-center gap-1 rounded-md border border-accent/30 bg-accent-soft px-2.5 py-0.5 text-[12px] text-accent-ink">
                      #{t}
