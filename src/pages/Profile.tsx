...
import { Copy, Check, Pencil, X, Camera, Package, ShieldCheck, Gift, ChevronRight } from "lucide-react";
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1 space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              {editing ? (
                <div className="flex items-center gap-1.5 flex-1">
                  <input
                    autoFocus
                    className="input h-7 text-[12px] py-0 flex-1 min-w-0"
                  ...
                  <button
                    type="button"
                    onClick={saveEdit}
                    className="shrink-0 rounded-md bg-accent px-2.5 py-1 text-[10.5px] font-medium text-white"
                  >
                    Save
                  </button>
                  <button
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              />
              {isAdmin && (
                <span className="inline-flex items-center gap-1 rounded-md border border-accent bg-accent px-2 py-0.5 text-[10px] font-semibold text-white">
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-4 sm:grid-cols-4">
          {[
            ...
            <div key={label} className="space-y-0.5">
              <p className="text-[17px] font-semibold tabular-nums text-ink leading-none">
                {value}
              </p>
      ...
      {isOwn && (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[12px] font-semibold text-ink">Refer &amp; earn</p>
      ...
      {receivedFeedbacks.length > 0 && (
        <section className="card px-5 py-4 space-y-3">
          <div className="flex items-center justify-between">
            <h3 className="text-[13px] font-semibold text-ink">Ratings</h3>
      ...
      {receivedFeedbacks.length === 0 && (
        <div className="card px-5 py-6 text-center space-y-1">
          <p className="text-[12px] font-medium text-ink">No feedback yet</p>
          <p className="text-[11px] text-muted">
        ...
        <div className="flex border-b border-edge">
          {(["seller", "buyer", "listings"] as HistoryTab[]).map((t) => (
            <button
              ...
              type="button"
              onClick={() => setTab(t)}
              className={`flex-1 py-3 text-[10.5px] font-medium transition ${
                tab === t
        ...
        {tab === "listings" ? (
          myListings.filter(l => l.status !== "deleted").length === 0 ? (
            <div className="px-5 py-8 text-center space-y-3">
              <p className="text-[11px] text-muted">No listings yet.</p>
              {isOwn && (
                <Link to="/listings/new" className="btn-secondary inline-flex items-center gap-2 text-[11px]">
              ...
              {myListings.filter(l => l.status !== "deleted").map((l) => (
                <li key={l.id}>
                  <Link to={`/listings/${l.id}`} className="flex items-center justify-between gap-3 px-4 py-3 hover:bg-bg transition">
                    <div className="min-w-0 flex-1">
                      <p className="text-[11px] font-medium text-ink truncate">{l.title}</p>
                      <p className="text-[10px] text-muted">{l.priceAmount} {l.priceCurrency} · {l.ordersCount} orders</p>
                    </div>
                    <span className={`pill text-[10px] shrink-0 ${l.status === "active" ? "border-accent/30 bg-accent-soft text-accent-ink" : "border-edge bg-bg text-muted"}`}>
                      {l.status}
                    </span>
                  </Link>
                </li>
