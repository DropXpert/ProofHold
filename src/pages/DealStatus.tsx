  ...
  async function updateStatus() {
    ...
    try {
      if (isSupabaseConfiguredForClient()) {
        if (currentDeal.status === "awaiting_payment" && currentDeal.paymentTxHash) {
    ...
    <div className="space-y-5">
      ...
      <section className="card flex items-center justify-between gap-3 px-4 py-3.5">
        <div className="min-w-0 space-y-0.5">
          <p className="text-[12px] font-semibold text-ink">Need latest status?</p>
          <p className="text-[11.5px] leading-relaxed text-muted">
            Use this after payment, delivery, or confirmation.
          </p>
          {refreshMessage ? (
            <p className="text-[11.5px] text-muted" role="status">
              {refreshMessage}
            </p>
          ) : null}
        </div>
      ...
      {proofs.length > 0 ? (
        <section className="card space-y-4 px-5 py-5">
          <header className="flex items-center justify-between">
            ...
            <span className="text-[11.5px] text-muted">{proofs.length}</span>
          </header>
