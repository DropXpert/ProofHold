...
export default function AdminReview() {
  ...
  // If deal not in local store, pull from Supabase
  useEffect(() => {
    if (!deal && id) {
      loadFromSupabase();
    }
    ...
    return (
      <div className="space-y-5">
        ...
        <EmptyState
          ...
          title="Loading deal…"
          description="Fetching from Supabase"
          action={
    ...
    <div className="space-y-5">
      ...
      {canDecide ? (
        <form onSubmit={submit} className="card space-y-4 px-5 py-5 border-2 border-accent/40">
          ...
          {isProofWindow ? (
            <p className="rounded-md border border-warning/30 bg-warning/5 px-3 py-2 text-[12.5px] leading-relaxed text-ink">
