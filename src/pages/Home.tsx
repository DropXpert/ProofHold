...
const STEPS = [
  { icon: FilePlus2,    step: "1", label: "Seller creates a deal", detail: "Set price, deadline and delivery proof requirements." },
...
const QUICK_LINKS = [
  { to: "/create",   icon: FilePlus2,     label: "Your deals",  iconBg: "bg-accent-soft text-accent" },
  { to: "/listings", icon: Store,         label: "Marketplace", iconBg: "bg-warning/10 text-warning" },
  { to: "/find",     icon: Search,        label: "Find a deal", iconBg: "bg-accent-soft text-accent" },
  { to: "/support",  icon: MessageCircle, label: "Support",     iconBg: "bg-edge/60 text-muted" },
];
      ...
      <section className="space-y-2.5">
        ...
        <div className="rounded-xl border border-edge bg-surface shadow-receipt">
          {STEPS.map(({ icon: Icon, step, label, detail }, i) => (
            <div
              key={step}
              className={`flex items-start gap-3 px-4 py-3.5${i < STEPS.length - 1 ? " border-b border-edge/60" : ""}`}
            >
