...
export const useOfferStore = create<OfferState>((set) => {
  ...
  return {
    ...
    placeOffer: async (input) => {
      ...
      const { data, error } = await sb
        ...
        .insert({
          ...
          current_amount: Number(input.amount),
    ...
    counter: async (id, newAmount) => {
      patchLocal(id, { status: "countered", currentAmount: newAmount });
      await persist(id, { status: "countered", current_amount: Number(newAmount) });
    },
