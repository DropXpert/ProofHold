...
// Credit the originating listing's order count exactly once, when a deal first
// reaches `released` (a completed sale). The prev-status guard makes repeated
// calls — e.g. reconcileDeadlines re-running auto-release — idempotent. Partial
// refunds and refunds are disputes, so they don't count as orders.
    ...
    (set, get) => ({
      ...
      createDeal: async (input) => {
        if (isRemoteMode()) {
          ...
          const baseArgs = {
            ...
            p_price_amount: Number(input.priceAmount),
            p_price_currency: input.priceCurrency,
            p_seller_wallet_address: input.sellerWalletAddress,
            p_delivery_deadline_hours: input.deliveryDeadlineHours,
            p_confirmation_window_hours: input.confirmationWindowHours,
            p_required_delivery_proof: input.requiredDeliveryProof.trim(),
            p_refund_terms: input.refundTerms.trim(),
          ...
          if (res.error) throw new Error(res.error.message || "Failed to create deal");

          // Refresh local cache from DB so all selectors see the new deal
          await (get() as any).loadFromSupabase();
      ...
      submitPayment: async ({ dealId, buyerWalletAddress, paymentTxHash }) => {
        if (isRemoteMode()) {
          ...
          try {
            await supabase.functions.invoke("verify-payment", { body: { deal_id: dealId } });
          } catch (err) {
            console.warn("[XcrowHub] verify-payment invoke failed (cron will retry):", err);
          }
          await (get() as any).loadFromSupabase();
          return;
        }
      ...
      payDeal: async ({ dealId, buyerWalletAddress, paymentTxHash }) => {
        if (isRemoteMode()) {
          try {
            ...
            console.error("[XcrowHub] Remote payDeal failed, falling back to local:", err);
          }
        }
        ...
        if (isRemoteMode()) {
          try {
            ...
            await supabase.rpc("mark_delivered", {
              p_deal_id: dealId,
              p_delivery_note: deliveryNote.trim(),
            });
            await (get() as any).loadFromSupabase();
            return;
          } catch (err) {
            console.error("[XcrowHub] Remote markDelivered failed, falling back to local:", err);
          }
        }
        const deal = get().deals[dealId];
        if (!deal) return;
      ...
      confirmReceipt: async (dealId) => {
        if (isRemoteMode()) {
          try {
            ...
            await supabase.rpc("confirm_receipt", { p_deal_id: dealId });
            await (get() as any).loadFromSupabase();
            creditOrderOnRelease(prevStatus, get().deals[dealId]);
            return;
          } catch (err) {
            console.error("[XcrowHub] Remote confirmReceipt failed, falling back to local:", err);
          }
        }
      ...
      raiseQuery: async ({ dealId, raisedBy, reason, details }) => {
        if (isRemoteMode()) {
          try {
            const supabase = getSupabaseClient();
            await supabase.rpc("raise_query", {
              p_deal_id: dealId,
            ...
            console.error("[XcrowHub] Remote raiseQuery failed, falling back to local:", err);
          }
        }
        const deal = get().deals[dealId];
        if (!deal) return;
      ...
      submitProof: async ({
        ...
        if (isRemoteMode()) {
          try {
            ...
            console.error("[XcrowHub] Remote submitProof failed, falling back to local:", err);
          }
        }
        const deal = get().deals[dealId];
        if (!deal) return;
      ...
      resolveAfterProofDeadline: async (dealId) => {
        if (isRemoteMode()) {
          try {
            const supabase = getSupabaseClient();
            await supabase.rpc("resolve_after_proof_deadline", { p_deal_id: dealId });
            await (get() as any).loadFromSupabase();
            const updated = get().deals[dealId];
            if (updated && (updated.status === "released" || updated.status === "refunded")) {
              const currency = updated.priceCurrency;
              const network = currency === "NIM" ? "nimiq" : "evm";
              const isRelease = updated.status === "released";
              const to = isRelease ? updated.sellerWalletAddress : updated.buyerWalletAddress;
              const amt = updated.priceAmount;
              await supabase.functions.invoke("payout", {
                body: {
                  deal_id: dealId,
            ...
            console.error("[XcrowHub] Remote resolveAfterProofDeadline failed, falling back to local:", err);
          }
        }
      ...
      applyAdminDecision: async ({
        ...
        if (isRemoteMode()) {
          try {
            const supabase = getSupabaseClient();
            const prevStatus = get().deals[dealId]?.status;
            await supabase.rpc("apply_admin_decision", {
              p_deal_id: dealId,
              p_decision: decision,
              p_reason: reason.trim(),
              p_buyer_amount: buyerAmount ? Number(buyerAmount) : null,
              p_seller_amount: sellerAmount ? Number(sellerAmount) : null,
            ...
            if (decision === "partial_refund") {
              await supabase.functions.invoke("payout", { body: { deal_id: dealId, decision, leg: "seller" } });
              await supabase.functions.invoke("payout", { body: { deal_id: dealId, decision, leg: "buyer" } });
            } else {
              await supabase.functions.invoke("payout", { body: { deal_id: dealId, decision } });
            }
            return;
          } catch (err) {
            console.error("[XcrowHub] Remote applyAdminDecision failed, falling back to local:", err);
          }
        }
      ...
      cancelDeal: async (dealId) => {
        if (isRemoteMode()) {
          try {
            const supabase = getSupabaseClient();
            await supabase.rpc("cancel_deal", { p_deal_id: dealId });
            await (get() as any).loadFromSupabase();
            return;
          } catch (err) {
            console.error("[XcrowHub] Remote cancelDeal failed, falling back to local:", err);
          }
        }
        const deal = get().deals[dealId];
        if (!deal) return;
      ...
      expireDeal: async (dealId) => {
        if (isRemoteMode()) {
          try {
            const supabase = getSupabaseClient();
            await supabase.rpc("expire_deal", { p_deal_id: dealId });
            await (get() as any).loadFromSupabase();
            return;
          } catch (err) {
            console.error("[XcrowHub] Remote expireDeal failed, falling back to local:", err);
          }
        }
        const deal = get().deals[dealId];
        if (!deal) return;
      ...
      autoReleaseDeal: async (dealId) => {
        if (isRemoteMode()) {
          try {
            const supabase = getSupabaseClient();
            const prevStatus = get().deals[dealId]?.status;
            await supabase.rpc("auto_release_deal", { p_deal_id: dealId });
            await (get() as any).loadFromSupabase();
            creditOrderOnRelease(prevStatus, get().deals[dealId]);
            return;
          } catch (err) {
            console.error("[XcrowHub] Remote autoReleaseDeal failed, falling back to local:", err);
          }
        }
        const deal = get().deals[dealId];
        if (!deal) return;
      ...
      submitFeedback: async ({ dealId, fromAddr, toAddr, fromRole, rating, comment }) => {
        if (isRemoteMode()) {
          try {
            ...
            console.error("[XcrowHub] Remote submitFeedback failed, falling back to local:", err);
          }
        }
      ...
      seedDemoDeals: () => {
        ...
        const make = (
          ...
          confirmationDeadlineAt: patch.confirmationDeadlineAt,
      ...
      loadFromSupabase: async () => {
        ...
        const now = Date.now();
        if (now - lastLoadAt < LOAD_THROTTLE_MS) return;
        lastLoadAt = now;
