...
export const config = {
  nimiq: {
    network: (readString("VITE_NIMIQ_NETWORK", "test") as NimiqNetwork) ?? "test",
...
// Supabase (client-safe anon key + URL). When present the app can use remote mode
// (hybrid with the existing local demo as fallback when missing).
