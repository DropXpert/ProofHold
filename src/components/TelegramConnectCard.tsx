...
type State = "loading" | "linked" | "unlinked";
...
export function TelegramConnectCard() {
  ...
  async function refresh() {
    ...
    try {
      const { data } = await getSupabaseClient().rpc("get_my_notification_settings");
  ...
  useEffect(() => {
    refresh();
    // When the user returns from Telegram, re-check the link status.
    const onVisible = () => {
  ...
  async function connect() {
    setBusy(true);
    try {
      const { data: token, error } = await getSupabaseClient().rpc("create_telegram_link_token");
  ...
  async function disconnect() {
    setBusy(true);
    try {
      await getSupabaseClient().rpc("unlink_telegram");
      setState("unlinked");
      setUsername(null);
    } catch {
      // ignore
    ...
    return (
      <div className="card flex items-center gap-3 px-5 py-4">
        ...
        <button
          ...
          onClick={disconnect}
          disabled={busy}
          className="shrink-0 text-[11px] font-medium text-muted transition hover:text-ink disabled:opacity-50"
        >
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[12px] font-semibold text-ink">Connect Telegram</p>
