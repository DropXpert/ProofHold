...
export function BottomNav() {
  ...
  const tabs = [
    ...
    { to: "/create",   icon: Briefcase,     label: "Deals",   public: false },
    { to: "/listings", icon: Store,         label: "Market",  public: true  },
    { to: "/support",  icon: MessageCircle, label: "Support", public: false },
    ...
    <nav className="fixed inset-x-0 bottom-0 z-50 flex justify-center">
      <div className="w-full max-w-app border-t border-edge bg-surface/95 backdrop-blur-sm">
        <ul className="flex items-stretch">
          {tabs.map(({ to, icon: Icon, label, public: isPublic }) => {
              ...
              <li key={to} className="flex-1">
                ...
                >
                  ...
                  >
                    ...
                    {badge > 0 && (
                      <span className="absolute -top-1 -right-1.5 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-danger text-[8px] font-bold text-white">
                        {badge > 9 ? "9+" : badge}
                      </span>
                    )}
                  </span>
                  <span className={cn("text-[9px] font-medium tracking-wide", active && !locked && "text-accent")}>
                    {label}
                  </span>
                </Link>
              </li>
