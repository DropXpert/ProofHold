...
export function OfferCard({
  ...
  return (
    <div className="rounded-xl border border-edge bg-surface px-4 py-3.5 space-y-3">
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-baseline gap-2">
            <span className="text-[18px] font-bold tabular-nums text-ink leading-none">
              {offer.currentAmount}
            </span>
            <span className="text-[12px] font-medium text-muted">{offer.currency}</span>
            {wasCountered && (
      ...
      {/* Buyer's message */}
      {offer.message && (
        <p className="rounded-lg bg-bg px-3 py-2 text-[12.5px] leading-relaxed text-ink">
          {offer.message}
        </p>
      )}
      ...
      {sellerActs && countering && (
        <div className="space-y-2">
          <div className="relative">
            ...
            />
            <span className="absolute right-3 top-1/2 -translate-y-1/2 text-[12px] font-medium text-muted">
              {offer.currency}
            </span>
          </div>
      ...
      {viewer === "buyer" && displayStatus === "pending" && (
        <button
          ...
          onClick={onWithdraw}
          disabled={busy}
          className="btn-ghost w-full text-[12.5px] text-muted"
        >
          Withdraw offer
        </button>
      )}
