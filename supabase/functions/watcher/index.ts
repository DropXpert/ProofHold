...
// matches them to awaiting_payment deals (by data/memo → deal ID, or by amount),
// and calls pay_deal() to transition the deal to funds_held.
//
...
async function processTx(tx: any): Promise<boolean> {
  ...
  const valueLunas: number = tx.value ?? 0;
  const dataHex: string = tx.data ?? tx.extraData ?? "";
  ...
  const { count } = await supabase
    ...
    .eq("tx_hash", txHash);
  if (count && count > 0) {
    console.log(`[watcher/NIM] tx ${txHash.slice(0, 12)} already in ledger, skipping`);
    return false;
  }
  ...
  if (dealId) {
    // Exact match by deal ID embedded in memo
  ...
  if (!deal) {
    // Fallback: match by exact amount among awaiting_payment NIM deals
    const amountNim = valueLunas / 100_000;
    ...
    if (candidates) {
      deal = candidates.find(
        (d: any) => Math.abs(Number(d.price_amount) - amountNim) < 0.001
      ) ?? null;
    }
  }
  ...
  // Confirm via the server-only procedure (the watcher is the trusted verifier).
  ...
  if (rpcErr) {
    console.error(`[watcher/NIM] confirm_deal_payment failed for ${deal.id}:`, rpcErr.message);
    return false;
  }

  // Record incoming tx in ledger
  await supabase.from("transactions").insert({
    id: crypto.randomUUID(),
    deal_id: deal.id,
    direction: "in",
    network: "nimiq",
    amount: valueLunas / 100_000,
    currency: "NIM",
    from_addr: fromAddr,
    to_addr: NIM_CUSTODY_RAW.trim(),
    tx_hash: txHash,
    block_height: tx.blockNumber ?? tx.block_number ?? null,
    status: "confirmed",
  });
...
// Backstop for USDT payments made without going through the app's submit flow.
// ERC-20 transfers can't carry a deal-ID memo, so we match by exact amount.
...
async function processEvmLog(log: any): Promise<boolean> {
  ...
  // Match an awaiting_payment USDT deal by exact amount (oldest deadline first).
  const { data: candidates } = await supabase
    .from("deals")
    .select("id, price_amount, price_currency, status, payment_deadline_at")
    .eq("status", "awaiting_payment")
    .eq("price_currency", "USDT")
    .order("payment_deadline_at", { ascending: true })
    .limit(20);

  const deal = (candidates ?? []).find(
    (d: any) => decimalToUnits(String(d.price_amount), USDT_DECIMALS) === value,
  );
  if (!deal) {
    console.log(`[watcher/EVM] no awaiting USDT deal for ${value} units (tx ${txHash?.slice(0, 12)})`);
    return false;
  }
  ...
  if (rpcErr) {
    console.error(`[watcher/EVM] confirm_deal_payment failed for ${deal.id}:`, rpcErr.message);
    return false;
  }
  ...
  await supabase.from("transactions").insert({
    ...
    direction: "in",
    network: "evm",
    amount: Number(deal.price_amount),
    currency: "USDT",
    from_addr: from,
    to_addr: EVM_CUSTODY,
    tx_hash: txHash,
    block_height: parseInt(log.blockNumber, 16),
    status: "confirmed",
  });
