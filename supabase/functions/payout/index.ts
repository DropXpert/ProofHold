...
//   * AuthZ — caller must be an internal job, an admin, or a deal participant.
//     The function still derives recipient and amount server-side.
...
const NIM_CUSTODY = (Deno.env.get("NIM_CUSTODY_ADDR") ?? "").replace(/\s+/g, "").toLowerCase();
const EVM_CUSTODY = (Deno.env.get("EVM_CUSTODY_ADDR") ?? "").replace(/\s+/g, "").toLowerCase();
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (isCustodyRecipient(currency, toAddress)) {
      return json(
        { error: "Recipient is the custody address. Fix the deal wallet before payout." },
        409,
      );
    }
    ...
    if (existingTx?.tx_hash) {
      await applyDealPayoutPatch(deal_id, decision, leg, existingTx.tx_hash, feeApplies, feeAmount, feeBps);
    ...
    await applyDealPayoutPatch(deal_id, decision, leg, txHash, feeApplies, feeAmount, feeBps);
...
function isCustodyRecipient(currency: "NIM" | "USDT", address: string): boolean {
  const custody = currency === "NIM" ? NIM_CUSTODY : EVM_CUSTODY;
  return Boolean(custody) && normalizedAddress(address) === custody;
}

async function applyDealPayoutPatch(
  dealId: string,
  decision: Decision,
  leg: "seller" | "buyer" | undefined,
  txHash: string,
  feeApplies = false,
  feeAmount = "0",
  feeBps = 0,
