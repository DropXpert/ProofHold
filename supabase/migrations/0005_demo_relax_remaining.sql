-- 0005: Relax auth checks on remaining procedures for demo/anon-key mode.
-- create_deal, mark_delivered, raise_query, submit_proof, cancel_deal
-- all accept the actor address from the request body instead of requiring a JWT claim.
-- This matches the hybrid RoleToggle demo scenario in the frontend.
-- Re-enable strict checks by reverting to 0003 versions once auth Edge is live.
