-- 0004: Relax strict caller checks for demo / anon key usage
-- This allows the current hybrid demo (with RoleToggle) to work with just the anon key
-- while still using the real DB and procedures.
-- In production with proper auth Edge + JWTs, the wallet checks will be enforced again.
-- For now, we use the p_xxx_by parameters provided by the (demo) client.
